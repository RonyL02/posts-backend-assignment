...
type UserInfo = TestUser & { accessToken?: string, refreshToken?: string }
...
describe("Auth Tests", () => {
  ...
  test("Logout - invalidate refresh token", async () => {
    ...
    const responseForRefreshAfterLogout = await request(app).post("/auth/refreshToken")
      .set('Authorization', `JWT ${user.refreshToken}`);

    expect(responseForRefreshAfterLogout.statusCode).not.toBe(StatusCodes.OK);

    const responseForLogoutAfterLogout = await request(app).post("/auth/logout")
      .set('Authorization', `JWT ${user.refreshToken}`);
  ...
  test("Refresh token multiuple usage", async () => {
    ...
    const refreshWithValidTokenResponse = await request(app).post("/auth/refreshToken")
      .set('Authorization', `JWT ${user.refreshToken}`);
    ...
    const refreshWithInvalidatedTokenResponse = await request(app).post("/auth/refreshToken")
      .set('Authorization', `JWT ${user.refreshToken}`);

    expect(refreshWithInvalidatedTokenResponse.statusCode).not.toBe(StatusCodes.OK);

    const refreshWithValidTokenAfterFailResponse = await request(app).post("/auth/refreshToken")
